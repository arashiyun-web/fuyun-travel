...
import { replyLineText } from "@/lib/lineApi";
import {
  buildQuoteDraftText,
  buildQuoteOptions,
  cleanText,
...
async function handleEvent(event: LineEvent) {
  if (event.type !== "message" || event.message?.type !== "text" || !event.replyToken || !event.source?.userId) return;
  const reply = await handleText(event.source.userId, event.message.text || "");
